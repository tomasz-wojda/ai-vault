...
- **Contract:**
  1. `devops-daily-protocol` fetches ticket via `ai-worklog service jira ticket <KEY>`
...
- **Contract:**
  ...
  3. `devops-daily-protocol` runs `ai-worklog service jira log-time` without `--apply`
  4. After approval: reruns with `--apply`, then runs `ai-worklog service jira verify`
...
- **⚠️ Edge Case — Partial Day Work:** User worked on ticket across multiple sessions → time should aggregate. Check `ai-worklog service jira tempo` for existing entries.
...
- **Contract:** `jenkins-pipeline-architect` provides patterns for monitoring deployment jobs (async polling, timeout handling). `devops-daily-protocol` uses `ai-worklog service newrelic` to verify deployment health post-deploy.
...
- **Contract:** `devops-daily-protocol` runs `ai-worklog service newrelic violations` and `deployments <APP_ID>`. If a deployment correlates temporally with the alert, hands off to `jenkins-pipeline-architect` to inspect the deployment pipeline.
