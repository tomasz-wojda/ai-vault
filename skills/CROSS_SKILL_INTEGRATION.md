...
3. **Sibling layers coordinate via handoffs** — L2↔L3, L2↔L4 and L3↔L4 compose through the rules in section 2, not through direct calls
...
- **R-05** Every write routes through the Write Gate: file creation and edits, HTTP POST/PUT/DELETE, git operations, Tempo submission, and `--apply` on any framework command. L3 worklog edits and L4 pipeline edits are not exempt.
...
`PR.log` entry, and the checklist transition to `[~]` (R-13).
